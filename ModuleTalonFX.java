...
public class ModuleTalonFX extends Module {
    ...
    public ModuleTalonFX(int moduleId) {
        ...
        steerConfig.Feedback.FeedbackRemoteSensorID =
                (int) DriveConstants.moduleConfigs[moduleId][2];
    ...
    public void periodic() {
        ...
        if (TurboLogger.hasChanged("Drive_kV_" + moduleId))
            drivePIDConfig.kV =
                    TurboLogger.get("Drive_kV_" + moduleId, DriveConstants.kVDriveDefault);
        if (!drivePIDConfig.equals(new Slot0Configs()))
            driveMotor.getConfigurator().refresh(drivePIDConfig);
        ...
        if (TurboLogger.hasChanged("Steer_kV_" + moduleId))
            steerPIDConfig.kV =
                    TurboLogger.get("Steer_kV_" + moduleId, DriveConstants.kVSteerDefault);
        if (!steerPIDConfig.equals(new Slot0Configs()))
            steerMotor.getConfigurator().refresh(steerPIDConfig);
    ...
    public void resetPosition(SwerveModulePosition position) {
        ...
        driveMotor.setPosition(
                Radians.of(position.distanceMeters / DriveConstants.wheelRadius.in(Meters)));
    }
    ...
    public Distance getDrivePosition() {
        return Meters.of(
                driveMotor.getPosition().getValue().in(Radians)
                        * DriveConstants.wheelRadius.in(Meters));
    }

    public LinearVelocity getDriveVelocity() {
        return MetersPerSecond.of(
                driveMotor.getVelocity().getValue().in(RadiansPerSecond)
                        * DriveConstants.wheelRadius.in(Meters));
    }

    public LinearAcceleration getDriveAcceleration() {
        return MetersPerSecondPerSecond.of(
                driveMotor.getAcceleration().getValue().in(RadiansPerSecondPerSecond)
                        * DriveConstants.wheelRadius.in(Meters));
    }
